// --------------------------------------------------
// MX Packer Agent for RPG Maker MZ : Initiator Interaction
...
// From process.cwd()
const ITEMS_TO_PRELOAD_PATH = './js/plugins/mx-agent/items_to_preload.txt';
const SETTINGS_PATH = './js/plugins/mx-agent/settings.json';
// From current base directory
const AOT_CONVERTER_PATH = './aot_converter.html';
...
function validateItemsToPreload(itemsToPreloadString) {
    const baseDirectory = IOHelper.getMainDirectory();
    const preloadingItemArray = itemsToPreloadString.split('|');
...
function applySettings() {
    let documentSettings = getSettingsFromDocument();
    let settingsValidationResult = validateSettings(documentSettings);
    let itemsToPreloadValidationResult = validateItemsToPreload(documentSettings.ics.items_to_preload);
...
function start() {
    ...
    let settingsValidationResult = validateSettings(settings);
    let itemsToPreloadValidationResult = validateItemsToPreload(settings.ics.items_to_preload);
    ...
    if (settingsValidationResult[0] == false) {
        ...
        const settingsPath = path.resolve(IOHelper.getMainDirectory(), SETTINGS_PATH);
...
const itpPath = path.resolve(IOHelper.getMainDirectory(), ITEMS_TO_PRELOAD_PATH);

if (fs.existsSync(itpPath)) {
    let itp = fs.readFileSync(itpPath, { encoding: 'utf8', flag: 'r' });

    let itemsToPreloadValidationResult = validateItemsToPreload(itp);
